...
class PayOSPaymentSecurityTest extends TestCase
{
    ...
    public function test_payos_cancel_return_renders_result_and_updates_order_once(): void
    {
        [$order, $payment, $user] = $this->pendingOrder();
        ...
        $this->actingAs($user)->get($url)->assertOk()->assertSee('Đã hủy thanh toán!')->assertSee($order->order_code);
        $this->assertSame('cancelled', $order->fresh()->status);
        $this->assertSame('failed', $payment->fresh()->status);
        $this->get($url)->assertOk()->assertSee('Đã hủy thanh toán!');
    ...
    public function test_cancel_query_string_does_not_override_remote_pending_or_other_owner(): void
    {
        ...
        $url = route('student.checkout.failed', $order->order_code).'?cancel=true&status=CANCELLED';
        $this->actingAs($user)->get($url)->assertOk()->assertSee('Chưa xác nhận được trạng thái hủy');
        $this->assertSame('pending', $order->fresh()->status);
