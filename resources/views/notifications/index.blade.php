...
<x-dynamic-component :component="$layout" title="Thông báo" page-title="Thông báo">
...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        @if($unreadCount > 0)
            <form method="POST" action="{{ route('notifications.read-all') }}">
                @csrf
                <button type="submit" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        @forelse($notifications as $notification)
            <div class="border-b border-slate-100 last:border-b-0 {{ $notification->is_read ? 'bg-white' : 'bg-blue-50/40' }}">
                <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="min-w-0 flex-1">
                        ...
                        <h3 class="text-base font-semibold text-slate-900">{{ $notification->title }}</h3>
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                        ...
                        @elseif(! $notification->is_read)
                            <form method="POST" action="{{ route('notifications.read', $notification) }}">
                                @csrf
                                <button type="submit" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                                    Đánh dấu đã đọc
                                </button>
                            </form>
                        @endif
        ...
        @empty
            <div class="px-6 py-16 text-center">
                ...
                <h3 class="text-lg font-semibold text-slate-900">Chưa có thông báo</h3>
                <p class="mt-2 text-sm text-slate-500">Các thông báo từ hệ thống, giảng viên và quản trị viên sẽ hiển thị tại đây.</p>
            </div>
        @endforelse
    </div>
