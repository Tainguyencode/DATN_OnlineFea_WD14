                ...
                <form method="POST" action="{{ route('instructor.quizzes.questions.store', $quiz) }}"
                    ...
                    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px_120px_120px]">
                        ...
                        <label class="block">
                            ...
                            <input type="number" name="sort_order" value="{{ $quiz->questions->count() }}"
                                min="0"
                                class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-emerald-500">
                        </label>
                    </div>
                    ...
                    <button type="submit"
                        class="mt-4 rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white transition hover:bg-slate-800">
                        Them cau hoi
                    </button>
                </form>
            </section>
                    ...
                    <article class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span
                                        class="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-bold text-violet-700 ring-1 ring-violet-200">{{ $question->form_type }}</span>
                                    <span
                                        class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{{ $question->points }}
                                        diem</span>
                                    <span class="text-xs font-semibold text-slate-500">sort_order:
                                        {{ $question->sort_order }}</span>
                                </div>
                            ...
                            <div class="flex shrink-0 flex-wrap gap-2">
                                <details>
                                    ...
                                    <form method="POST"
                                        ...
                                        <button type="submit"
                                            class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700">Luu
                                            cau hoi</button>
                                    </form>
                                </details>
                                <form method="POST"
                        ...
                        <div class="mt-5 space-y-3">
                            @foreach ($question->options as $answer)
                                <form method="POST" action="{{ route('instructor.quiz-answers.update', $answer) }}"
                                    class="grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 lg:grid-cols-[minmax(0,1fr)_120px_140px_90px_auto] lg:items-center">
                                    @csrf
                                    @method('PUT')
                                    <input type="text" name="answer_text" value="{{ $answer->option_text }}"
                                        required
                                        class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-500">
                                    <input type="number" name="sort_order" value="{{ $answer->sort_order }}"
                                        min="0"
                                        class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-500">
                                    ...
                                    <button type="submit"
                                        class="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-bold text-white hover:bg-emerald-700">Luu</button>
                                    <button form="delete-answer-{{ $answer->id }}" type="submit"
                                        class="rounded-lg border border-rose-200 px-3 py-2 text-sm font-bold text-rose-700 hover:bg-rose-50">Xoa</button>
                                </form>
                                <form id="delete-answer-{{ $answer->id }}" method="POST"
                                    action="{{ route('instructor.quiz-answers.destroy', $answer) }}"
                                    onsubmit="return confirm('Xoa dap an nay?')" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            @endforeach
                            ...
                            <form method="POST"
                                ...
                                class="grid gap-3 rounded-lg border border-dashed border-slate-300 p-3 lg:grid-cols-[minmax(0,1fr)_120px_140px_auto] lg:items-center">
                                @csrf
                                ...
                                <button type="submit"
                                    class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white hover:bg-slate-800">Them
                                    dap an</button>
                            </form>
                        </div>
                    </article>
