<x-instructor-layout :title="'Quiz - ' . $lesson->title" page-title="Quan ly quiz" :breadcrumb="$course->title">
    ...
    <div class="space-y-6 pb-28">
        ...
        @if ($quiz)
            ...
            @if ($publishedMappings->isNotEmpty())
                <section class="rounded-lg border border-amber-200 bg-amber-50 p-5 shadow-sm sm:p-6" aria-labelledby="invalid-question-heading">
                    <div>
                        <h3 id="invalid-question-heading" class="text-lg font-bold text-amber-950">Báo cáo câu hỏi đã xuất bản</h3>
                        <p class="mt-1 text-sm leading-6 text-amber-900">Yêu cầu sẽ chuyển tới admin duyệt. Câu hỏi chỉ bị loại khỏi điểm sau khi được phê duyệt.</p>
                    </div>
                    <div class="mt-4 space-y-3">
                        @foreach ($publishedMappings as $publishedMapping)
                            @php
                                $publishedInvalidation = $publishedInvalidations->get($publishedMapping->id);
                                $publishedCounts = $publishedInvalidationCounts->get($publishedMapping->id, ['completed' => 0, 'in_progress' => 0]);
                                $canRequestInvalidation = !$publishedInvalidation || $publishedInvalidation->status === \App\Models\QuizVersionQuestionInvalidation::STATUS_REJECTED;
                            @endphp
                            <details class="rounded-lg border border-amber-200 bg-white p-4">
                                <summary class="cursor-pointer list-none font-bold text-slate-900">
                                    Câu {{ $loop->iteration }} — {{ $publishedMapping->questionVersion?->question }}
                                    @if ($publishedInvalidation?->status === \App\Models\QuizVersionQuestionInvalidation::STATUS_ACTIVE)
                                        <span class="ml-2 rounded-full bg-rose-100 px-2 py-1 text-xs text-rose-800">Đã hủy</span>
                                    @elseif ($publishedInvalidation?->status === \App\Models\QuizVersionQuestionInvalidation::STATUS_PENDING)
                                        <span class="ml-2 rounded-full bg-amber-100 px-2 py-1 text-xs text-amber-800">Đang chờ admin</span>
                                    @endif
                                </summary>
                                <div class="mt-3 border-t border-slate-100 pt-3 text-sm text-slate-600">
                                    <p>Phiên bản V{{ $publishedVersion->version }} · {{ $publishedCounts['completed'] }} lượt đã hoàn thành · {{ $publishedCounts['in_progress'] }} lượt đang làm.</p>
                                    @if ($publishedInvalidation?->status === \App\Models\QuizVersionQuestionInvalidation::STATUS_ACTIVE)
                                        <p class="mt-2 font-bold text-rose-700">Câu hỏi đã bị hủy — các lượt làm bị ảnh hưởng đã được xử lý regrade.</p>
                                    @elseif ($publishedInvalidation?->status === \App\Models\QuizVersionQuestionInvalidation::STATUS_PENDING)
                                        <p class="mt-2 font-semibold text-amber-800">Yêu cầu đang chờ admin phê duyệt.</p>
                                    @elseif ($canRequestInvalidation)
                                        <form method="POST" action="{{ route('instructor.quiz-version-questions.invalidations.store', $publishedMapping) }}" class="mt-3 space-y-3">
                                            @csrf
                                            <label class="block">
                                                <span class="mb-1 block text-xs font-bold text-slate-700">Lý do hủy câu hỏi</span>
                                                <textarea name="reason" rows="3" minlength="5" maxlength="5000" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-amber-500 focus-visible:ring-2 focus-visible:ring-amber-500/30">{{ old('reason') }}</textarea>
                                            </label>
                                            <button type="submit" class="inline-flex min-h-10 items-center justify-center rounded-lg bg-amber-700 px-4 py-2 text-sm font-bold text-white transition hover:bg-amber-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500">Gửi yêu cầu hủy</button>
                                        </form>
                                    @endif
                                </div>
                            </details>
