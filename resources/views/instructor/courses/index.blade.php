<x-instructor-layout title="Khóa học của tôi" page-title="Khóa học của tôi" breadcrumb="Quản lý danh mục khóa học, bản nháp và trạng thái duyệt">
    ...
    @if($courses->isEmpty())
        <div class="rounded-lg border border-dashed border-slate-300 bg-white px-6 py-14 text-center shadow-sm">
            ...
            <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                Bắt đầu bằng một bản nháp khóa học. Bạn có thể bổ sung thumbnail, giá bán và nội dung bài học trước khi gửi duyệt.
            </p>
            <a href="{{ route('instructor.courses.create') }}"
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($courses as $course)
                            ...
                            <tr class="align-middle transition-colors duration-200 hover:bg-slate-50">
                                ...
                                <td class="px-5 py-4 text-slate-500">{{ $course->created_at?->format('d/m/Y') }}</td>
                                <td class="px-5 py-4">
                ...
                <article class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                    <div class="aspect-video bg-slate-100">
                        @if($course->thumbnail)
                            <img src="{{ asset('storage/'.$course->thumbnail) }}" alt="{{ $course->title }}" class="h-full w-full object-cover">
                        @else
                            <div class="flex h-full w-full items-center justify-center bg-gradient-to-br from-slate-800 to-emerald-700 text-sm font-bold text-white">Fea LMS</div>
                        @endif
                    </div>
                    <div class="space-y-4 p-4">
                        <div>
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <span class="shrink-0 rounded-full border px-2.5 py-1 text-xs font-bold {{ $statusClass }}">
                                    {{ $statusOptions[$course->status] ?? $course->status }}
                                </span>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 rounded-lg bg-slate-50 p-3 text-sm">
                            <div>
                                ...
                                <strong class="text-slate-950">{{ $formatPrice($discountPrice ?? $course->price) }}</strong>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <form method="POST" action="{{ route('instructor.courses.destroy', $course) }}" onsubmit="return confirm('Bạn chắc chắn muốn xóa hoặc lưu trữ khóa học này?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full rounded-lg border border-rose-200 px-3 py-2 text-center text-xs font-bold text-rose-700">Xóa</button>
                            </form>
                        </div>
                    </div>
                </article>
