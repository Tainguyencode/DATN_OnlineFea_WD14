<x-instructor-layout title="Đánh giá học viên" pageTitle="Đánh giá học viên" breadcrumb="Giảng viên / Đánh giá">
    <div class="space-y-6">
        <form method="GET" class="grid gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <select name="status" class="cursor-pointer rounded-xl border-slate-300 text-sm dark:border-slate-700 dark:bg-slate-950"><option value="">Tất cả trạng thái</option>@foreach([\App\Enums\ReviewStatus::Approved, \App\Enums\ReviewStatus::Hidden] as $option)<option value="{{ $option->value }}" @selected($status === $option->value)>{{ $option->label() }}</option>@endforeach</select>
