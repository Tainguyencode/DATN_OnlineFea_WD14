...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    <x-stat-card label="Doanh thu gộp" :value="number_format($totalGross, 0, ',', '.') . 'đ'" color="blue"
        icon='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>' />
    <x-stat-card label="Lợi nhuận nền tảng" :value="number_format($totalCommission, 0, ',', '.') . 'đ'" color="emerald"
        icon='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 11l3-3m0 0l3 3m-3-3v8m0-13a9 9 0 110 18 9 9 0 010-18z"/></svg>' />
    <x-stat-card label="Chi trả giáo viên" :value="number_format($totalInstructorEarning, 0, ',', '.') . 'đ'" color="amber"
...
<div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
    <div class="p-5 border-b border-slate-100">
        <h2 class="font-bold text-slate-900">Chi tiết doanh thu theo tháng</h2>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="text-center px-6 py-3 font-semibold text-slate-600">Số đơn</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($monthly as $row)
                    <tr class="hover:bg-slate-50">
                        <td class="px-6 py-4 font-medium text-slate-900">{{ $row->month }}</td>
                        <td class="px-6 py-4 text-center text-slate-600 font-medium">{{ $row->count }}</td>
                        <td class="px-6 py-4 text-right font-medium text-slate-700">{{ number_format($row->total, 0, ',', '.') }}đ</td>
                        <td class="px-6 py-4 text-right font-semibold text-emerald-600">{{ number_format($row->commission ?? 0, 0, ',', '.') }}đ</td>
                        <td class="px-6 py-4 text-right font-semibold text-amber-600">{{ number_format($row->instructor_earning ?? 0, 0, ',', '.') }}đ</td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center text-slate-500">Chưa có dữ liệu.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
