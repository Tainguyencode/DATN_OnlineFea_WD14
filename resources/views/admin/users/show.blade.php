...
<div class="space-y-6">
    ...
    @if ($user->role === 'instructor')
        <section class="rounded-xl border border-rose-200 bg-rose-50/50 p-5 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="text-base font-bold text-slate-900 flex items-center gap-2">
                        ...
                        Chiết khấu & Tài khoản Ngân hàng (Giảng viên)
                    </h3>
                    <p class="text-xs text-slate-500 mt-0.5">Tỷ lệ áp dụng hiện tại: <strong class="font-bold text-rose-600">{{ number_format($user->getCommissionRate(), 1) }}%</strong> ({{ $user->commission_rate !== null ? 'Tỷ lệ riêng' : 'Mặc định hệ thống' }})</p>
                </div>
                <a href="{{ route('admin.commissions.index', ['search' => $user->email]) }}" class="inline-flex items-center gap-2 rounded-lg bg-rose-600 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-rose-700 transition">
                    Quản lý chiết khấu
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </a>
            </div>
            @if ($user->bank_name || $user->bank_account_number)
                <div class="mt-4 grid gap-3 sm:grid-cols-3 border-t border-rose-200/60 pt-4 text-xs">
                    <div>
                        <span class="font-semibold text-slate-500">Ngân hàng:</span>
                        <strong class="block text-slate-900 font-bold mt-0.5">{{ $user->bank_name }}</strong>
                    </div>
                    <div>
                        <span class="font-semibold text-slate-500">Số tài khoản:</span>
                        <strong class="block text-slate-900 font-mono font-bold mt-0.5">{{ $user->bank_account_number }}</strong>
                    </div>
                    <div>
                        <span class="font-semibold text-slate-500">Chủ tài khoản:</span>
                        <strong class="block text-slate-900 uppercase font-bold mt-0.5">{{ $user->bank_account_name }}</strong>
                    </div>
                </div>
            @endif
        </section>
