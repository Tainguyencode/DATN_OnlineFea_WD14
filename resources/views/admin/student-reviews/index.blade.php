                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($reviews as $review)
                            <tr class="align-top">
                                ...
                                <td class="px-4 py-4"><div class="flex flex-col items-end gap-2"><a href="{{ route('admin.student-reviews.show', $review) }}" class="font-bold text-indigo-700 hover:underline dark:text-indigo-300">Chi tiết</a>@if($review->isHidden())<form method="POST" action="{{ route('admin.student-reviews.restore', $review) }}">@csrf @method('PATCH')<button class="cursor-pointer font-bold text-emerald-700 hover:underline dark:text-emerald-300">Hiện lại</button></form>@else<form method="POST" action="{{ route('admin.student-reviews.hide', $review) }}">@csrf @method('PATCH')<button class="cursor-pointer font-bold text-slate-700 hover:underline dark:text-slate-300">Ẩn</button></form>@endif<form method="POST" action="{{ route('admin.student-reviews.destroy', $review) }}" onsubmit="return confirm('Xóa mềm đánh giá này? Bạn có thể khôi phục trong cơ sở dữ liệu nếu cần.')">@csrf @method('DELETE')<button class="cursor-pointer font-bold text-rose-700 hover:underline dark:text-rose-300">Xóa</button></form></div></td>
                            </tr>
                        @empty
