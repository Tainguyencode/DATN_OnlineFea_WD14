                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($reviews as $review)
                            <tr class="align-top">
                                ...
                                <td class="px-4 py-4"><span class="font-bold text-amber-600">{{ $review->rating }}/5 sao</span><p class="mt-1 max-w-sm text-slate-600 dark:text-slate-300">{{ \Illuminate\Support\Str::limit($review->comment, 120) }}</p>@if($review->instructor_reply)<span class="mt-2 inline-flex rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-bold text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300">Đã phản hồi</span>@endif</td>
