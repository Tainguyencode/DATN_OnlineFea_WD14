                    ...
                    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="mt-6 space-y-5">
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            <div>
                                ...
                                <input name="name" value="{{ old('name', $user->name) }}" required class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Username</label>
                                <input name="username" value="{{ old('username', $user->username) }}" required class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Số điện thoại</label>
                                <input name="phone" value="{{ old('phone', $user->phone) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Avatar</label>
                                <input type="file" name="avatar" accept="image/png,image/jpeg,image/webp" class="w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm file:mr-3 file:rounded-xl file:border-0 file:bg-blue-50 file:px-3 file:py-2 file:font-bold file:text-[#0056D2]">
                            </div>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold text-slate-700">Giới thiệu</label>
                            <textarea name="bio" rows="4" class="w-full resize-none rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">{{ old('bio', $user->bio) }}</textarea>
                        </div>
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <form method="POST" action="{{ route('profile.email.update') }}" class="mt-5 space-y-4">
                            @csrf
                            @method('PUT')
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            <input type="password" name="current_password" required placeholder="Mật khẩu hiện tại" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            <button class="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-bold text-white">Cập nhật email</button>
                        </form>
                    </div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <form method="POST" action="{{ route('profile.password.update') }}" class="mt-5 space-y-4">
                            ...
                            @method('PUT')
                            <input type="password" name="current_password" required placeholder="Mật khẩu hiện tại" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            <input type="password" name="password" required placeholder="Mật khẩu mới" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            <input type="password" name="password_confirmation" required placeholder="Xác nhận mật khẩu mới" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#0056D2] focus:ring-4 focus:ring-blue-500/10">
                            <button class="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-bold text-white">Cập nhật mật khẩu</button>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <span class="rounded-full px-3 py-1 text-xs font-bold {{ $user->two_factor_enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">{{ $user->two_factor_enabled ? 'ON' : 'OFF' }}</span>
                    </div>
                    ...
                    @if(! $user->two_factor_enabled)
                        ...
                        @if(session('two_factor_pending'))
                            <form method="POST" action="{{ route('profile.two-factor.enable') }}" class="mt-4 flex gap-3">
                                ...
                                <button class="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-bold text-white">Xác nhận</button>
                            </form>
                        @endif
                    @else
                        <form method="POST" action="{{ route('profile.two-factor.disable') }}" class="mt-5 space-y-3">
                            @csrf
                            @method('DELETE')
                            <input type="password" name="current_password" required placeholder="Mật khẩu hiện tại" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-red-400 focus:ring-4 focus:ring-red-500/10">
                    ...
                    <div class="mt-5 space-y-4">
                        @forelse($activityLogs as $log)
                            <div class="flex gap-3">
                                <div class="mt-1 h-2.5 w-2.5 rounded-full bg-[#0056D2]"></div>
                                <div>
