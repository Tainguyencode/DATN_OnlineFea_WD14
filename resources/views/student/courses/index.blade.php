...
@if($enrollments->isEmpty())
    <div class="rounded-2xl border border-dashed border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#161615] p-12 text-center shadow-sm">
        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400">
        ...
        <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Khám phá catalog khóa học đã được duyệt và bắt đầu học ngay.</p>
...
@else
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
        @foreach($enrollments as $enrollment)
            ...
            @endphp
            <article class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#161615] shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-lg flex flex-col h-full">
                ...
                <div class="p-5 flex flex-col flex-grow">
                    <div class="flex items-center justify-between gap-3">
                        <span class="truncate text-xs font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">{{ $course->category?->name ?? 'Khóa học' }}</span>
                        <span class="rounded-full bg-emerald-50 dark:bg-emerald-950/30 px-2.5 py-1 text-xs font-bold text-emerald-700 dark:text-emerald-400 ring-1 ring-emerald-200 dark:ring-emerald-800/50">
                    ...
                    <div class="mt-auto pt-5">
                        ...
                        @if($enrollment->completed_at)
                            <div class="mt-3">
                                <span class="ui-badge-success">Hoàn thành</span>
                            </div>
                        @endif
