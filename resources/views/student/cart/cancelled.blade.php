...
<body class="min-h-screen bg-slate-50 font-sans text-slate-900 dark:bg-slate-950 dark:text-white">
    <main data-refresh-on-history class="flex min-h-screen items-center justify-center px-4 py-10">
        <div class="w-full max-w-xl space-y-6 text-center">
            ...
            <h1 class="text-2xl font-black">{{ $order->status === 'cancelled' ? 'Đã hủy thanh toán!' : 'Bạn đã dừng thanh toán' }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">
                {{ $order->status === 'cancelled' ? 'Đơn hàng đã được hủy. Khóa học chưa được kích hoạt từ đơn này.' : 'Chưa xác nhận được trạng thái hủy từ PayOS. Đơn hàng vẫn đang chờ thanh toán; vui lòng kiểm tra lại trước khi tạo giao dịch khác.' }}
            </p>
            <section class="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 text-left shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <div class="flex justify-between gap-4 text-sm"><span>Trạng thái</span><strong>{{ $order->status === 'cancelled' ? 'Đã hủy' : 'Chờ xác nhận' }}</strong></div>
            </section>
