...
@if($cart->items->isEmpty())
    <div class="bg-white rounded-2xl border border-slate-200 p-16 text-center">
        ...
        <a href="{{ route('home') }}#courses" class="inline-block mt-4 text-indigo-600 font-medium hover:underline">Tiếp tục mua sắm →</a>
    </div>
@else
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            @foreach($cart->items as $item)
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-5 flex items-center gap-4">
                    ...
                    <div class="text-right">
                        <div class="font-bold text-indigo-600">{{ number_format($price, 0, ',', '.') }}đ</div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 h-fit sticky top-24">
            ...
            <div class="flex justify-between font-bold text-lg text-slate-900 border-t border-slate-200 pt-4 mt-4 mb-6">
                <span>Tổng cộng</span>
                <span class="text-indigo-600">{{ number_format($total, 0, ',', '.') }}đ</span>
            </div>
            <form method="POST" action="{{ route('student.cart.checkout') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="payment_method" required class="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm bg-white focus:ring-2 focus:ring-indigo-500 outline-none">
                        <option value="vnpay">VNPay</option>
                        <option value="momo">MoMo</option>
                        <option value="bank_transfer">Chuyển khoản</option>
                    </select>
                </div>
                <button type="submit" class="w-full bg-indigo-600 text-white font-semibold py-3 rounded-xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-200">
                    Thanh toán ngay
                </button>
            </form>
        </div>
