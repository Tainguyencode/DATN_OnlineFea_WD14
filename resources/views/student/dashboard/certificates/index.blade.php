        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @foreach($certificates as $certificate)
                <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <div class="mt-5 grid grid-cols-2 gap-2"><a target="_blank" rel="noopener" href="{{ route('student.certificates.pdf', $certificate) }}" class="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#0056D2] px-3 text-sm font-bold text-white">Xem</a><a href="{{ route('student.certificates.pdf', [$certificate, 'download' => 1]) }}" class="inline-flex min-h-10 items-center justify-center rounded-xl border border-slate-200 px-3 text-sm font-bold hover:bg-slate-50 dark:border-slate-700">Tải xuống</a></div>
                </article>
            @endforeach
        </div>
